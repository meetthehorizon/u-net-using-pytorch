...
class LungDataset(Dataset):
	def __init__(self, path='data/train', transform=None, num=None):
		...
		mask_dir = path + '/mask/'
		self.image_paths = [image_dir + f for f in os.listdir(image_dir) if f.endswith('.png')]
		self.mask_paths = [mask_dir + f for f in os.listdir(mask_dir) if f.endswith('.jpeg')]
	...
	def __getitem__(self, index):
		...
		image, mask = Image.open(self.image_paths[index]).convert('L'), Image.open(self.mask_paths[index]).convert('L')
